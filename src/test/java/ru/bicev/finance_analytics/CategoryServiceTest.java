...
public class CategoryServiceTest {
    ...
    void testGetUserCategories_noAccountId() {
        List<Category> categories = List.of(
                Category.builder().id(UUID.randomUUID()).user(user).name("Food").build());

        when(categoryRepository.findAllByUserId(user.getId()))
                .thenReturn(categories);

        List<Category> result = categoryService.getUserCategories(null);
    ...
    void testGetUserCategories_withAccountId() {
        UUID accountId = UUID.randomUUID();
        List<Category> categories = List.of(
                Category.builder().id(UUID.randomUUID()).user(user).name("Transport").build());

        when(categoryRepository.findAllByUserIdAndAccountId(user.getId(), accountId))
                .thenReturn(categories);

        List<Category> result = categoryService.getUserCategories(accountId);
    ...
    void testGetCategoriesByType_success() {
        List<Category> categories = List.of(
                Category.builder().id(UUID.randomUUID()).user(user).name("Salary").type(CategoryType.INCOME).build());

        when(categoryRepository.findAllByUserIdAndType(user.getId(), CategoryType.INCOME))
                .thenReturn(categories);

        List<Category> result = categoryService.getCategoriesByType(CategoryType.INCOME);
